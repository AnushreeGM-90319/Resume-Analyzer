...
import spacy
import os
os.environ['SPACY_DATA'] = r'C:\PROJECT\Resume-Analyzer\venv\Lib\site-packages\spacy'
    ...
    if choice == 'Normal User':
        ...
        if pdf_file is not None:
            ...
            if resume_data:
                ...
                ##  recommendation
                ds_keyword = ['tensorflow','keras','pytorch','machine learning','deep Learning','flask','streamlit', 'data analysis', 'data visualization', 'statistical modeling', 'data mining', 'natural language processing', 'computer vision']
                web_keyword = ['react', 'django', 'node jS', 'react js', 'php', 'laravel', 'magento', 'wordpress',
                               'javascript', 'angular js', 'c#', 'flask', 'html', 'css', 'api', 'restful api']
                android_keyword = ['android','android development','flutter','kotlin','xml','kivy','java', 'mobile development', 'android sdk']
                ios_keyword = ['ios','ios development','swift','cocoa','cocoa touch','xcode', 'objective-c', 'ios sdk', 'app store']
                uiux_keyword = ['ux','adobe xd','figma','zeplin','balsamiq','ui','prototyping','wireframes','storyframes','adobe photoshop','photoshop','editing','adobe illustrator','illustrator','adobe after effects','after effects','adobe premier pro','premier pro','adobe indesign','indesign','wireframe','solid','grasp','user research','user experience', 'interaction design', 'visual design']
                ...
                if any(skill in ds_keyword for skill in all_skills):
                    job_titles.append('Data Scientist')
                if any(skill in web_keyword for skill in all_skills):
                    job_titles.append('Web Developer')
                if any(skill in android_keyword for skill in all_skills):
                    job_titles.append('Android Developer')
                if any(skill in ios_keyword for skill in all_skills):
                    job_titles.append('iOS Developer')
                if any(skill in uiux_keyword for skill in all_skills):
                    job_titles.append('UI/UX Designer')
                ...
                if not job_titles:
                    ...
                    st.success(f"**Based on your skills, the best matching job title is: {best_job_title}**")
                    
                    user_pref = st.text_input("If you have a different preference for a job title, enter it here (otherwise, press Enter):",value=best_job_title)
                    
                    
                    recommended_skills = []
                    rec_course = []
                    
                    if user_pref.lower() == 'data scientist' :
                        recommended_skills = ['Data Visualization','Predictive Analysis','Statistical Modeling','Data Mining','Clustering & Classification','Data Analytics','Quantitative Analysis','Web Scraping','ML Algorithms','Keras','Pytorch','Probability','Scikit-learn','Tensorflow',"Flask",'Streamlit','NLP','Computer Vision']
                        recommended_keywords = st_tags(label='### Recommended skills for you.',
                         text='Recommended skills generated from System',value=recommended_skills,key = '2')
                        st.markdown('''<h4 style='text-align: left; color: #1ed760;'>Adding this skills to resume will boost🚀 the chances of getting a Job💼</h4>''',unsafe_allow_html=True)
                        rec_course = course_recommender(ds_course)

                    elif user_pref.lower() == 'web developer' :
                        recommended_skills = ['React','Django','Node JS','React JS','php','laravel','Magento','wordpress','Javascript','Angular JS','c#','Flask','SDK','HTML','CSS','API','RESTful API']
                        recommended_keywords = st_tags(label='### Recommended skills for you.',
                    ...
                    elif user_pref.lower() == 'android developer':
                        recommended_skills = ['Android','Android development','Flutter','Kotlin','XML','Java','Kivy','GIT','SDK','SQLite','Mobile Development','Android SDK']
                        recommended_keywords = st_tags(label='### Recommended skills for you.',
                    ...
                    elif user_pref.lower() == 'ios developer':
                        recommended_skills = ['IOS','IOS Development','Swift','Cocoa','Cocoa Touch','Xcode','Objective-C','SQLite','Plist','StoreKit',"UI-Kit",'AV Foundation','Auto-Layout','IOS SDK','App Store']
                        recommended_keywords = st_tags(label='### Recommended skills for you.',
                    ...
                    elif user_pref.lower() == 'ui/ux designer':
                        recommended_skills = ['UI','User Experience','Adobe XD','Figma','Zeplin','Balsamiq','Prototyping','Wireframes','Storyframes','Adobe Photoshop','Editing','Illustrator','After Effects','Premier Pro','Indesign','Wireframe','Solid','Grasp','User Research','Interaction Design','Visual Design']
                        recommended_keywords = st_tags(label='### Recommended skills for you.',
